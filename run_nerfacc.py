...
# TODO move it to the vanilla nerf
...
# TEST mode only
def render_video(radiance_field, estimator, poses, H, W, intrinsics, filename, args, near, far, fps=25):
    radiance_field.eval()
    estimator.eval()
...
# TEST mode only
def optimize_camera_embedding(radiance_field, estimator, image, pose, H, W, intrinsic, args):
    radiance_field.set_camera_embed_zeros()
    optimizer = torch.optim.Adam(params=(radiance_field.embedded_cam,), lr=5e-1)
    ...
    for i in range(100):
        ...
        if psnr > max_psnr:
            max_psnr = psnr
            best_embedded_cam = radiance_field.embedded_cam.detach().clone()
...
def render_images_with_metrics(radiance_field, estimator, count, indices, images, depths, valid_depths, poses, H, W, intrinsics, lpips_alex, args, far, near,\
    ...
    for n, img_idx in enumerate(img_i):
        ...
        if args.input_ch_cam > 0:
            if not use_train_images:
                ...
                if with_test_time_optimization:
                    optimize_camera_embedding(radiance_field, estimator,target, pose, H, W, intrinsic, args)
        ...
        with torch.no_grad():
            ...
            depths_res[n] = (pre_depths / far).unsqueeze(0).cpu()
...
# will be moved into render_image_with_grid
...
# regarded depth sampling, not right now
...
# this part will be implemented inside render_image_with_grid (similar to nerfacc example)
...
def train_nerf(images, depths, valid_depths, poses, intrinsics, i_split, args, scene_sample_params, lpips_alex, gt_depths, gt_valid_depths):
    ...
    # Load checkpoints if we need to resume training
    ckpt = load_checkpoint(args, device)
    if ckpt is not None:
        start = ckpt['global_step']
        optimizer.load_state_dict(ckpt['optimizer_state_dict'])
...
def run_nerf():
    ...
    if "test" in args.task:
        ...
        render_video(radiance_field, estimator, vposes, H, W, vintrinsics, str(0), args, near, far)
